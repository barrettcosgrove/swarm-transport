...
Payload proximity	per unit distance, shared	starts ≈ progress coef, annealed to 0 over first ~30% of training	—	Exploration crutch only
Obstacle/wall collision	per-step, private	≈ −0.1	—	Minor deterrent
Constraint chain holds: 75 < 80 < 100 and 70 < 100.

Notes:

Time penalty derives from an inequality: episode_length × |R_time| < |R_capture|. With 250 steps and R_capture = 100, that means |R_time| < 0.4; 0.3 leaves real headroom.
Health loss is sized via k × health_pool rather than k × drain_rate, so the cumulative penalty depends only on total HP lost, not on how fast the predator happens to drain it. Keeps the number meaningful regardless of later predator tuning. Structurally, health loss is a dense signal building toward a sparse terminal penalty — the exact mirror of progress's relationship to the success bonus.
...
No predator-distance shaping. The predator guards the payload, so a permanent distance-from-predator reward would be a permanent incentive to avoid the payload — directly undermining the objective. The health penalty already supplies "don't get caught," and it only fires when capture is actually happening rather than constantly pulling agents backward. The guard mechanic is the tension; a second reward term manufacturing more of it is counterproductive.
